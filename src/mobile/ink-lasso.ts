...
//
// 与 ink-erase.ts 同一设计原则：全程只走公开契约（serialize / deserialize /
...
/** 编辑器 serialize 后的包围盒（PDF 坐标），用于粗筛与屏幕换算。 */
...
export function lassoHitEditors(engine: InkEngine, pageIndex: number, poly: ScreenPoint[]): any[] {
	...
	for (const editor of editors) {
		...
		const lines: ArrayLike<number>[] = serial?.paths?.lines ?? [];
		if (lines.some((line) => strokeInPolygon(line, poly))) hits.push(editor);
	}
	return hits;
}
...
export async function moveEditorBy(
	...
	const lines: ArrayLike<number>[] = serial?.paths?.lines ?? [];
	if (!lines.length) return { ok: false, rebuilt: null, error: '编辑器没有笔画' };
	...
	if (Array.isArray(serial.rect) && serial.rect.length === 4) {
		rebuiltData.rect = [serial.rect[0] + dx, serial.rect[1] + dy, serial.rect[2] + dx, serial.rect[3] + dy];
	}
